<template>
    <div class="w-screen transition-all duration-300 h-[90vh] md:h-screen overflow-hidden bg-white md:flex relative">
        <div :class="error.length > 0 ? 'w-auto px-5 duration-500' : 'duration-500'"
        ...
        <div v-if="MailTap" class="w-full h-full flex justify-center items-center  absolute backdrop-blur-md z-30">
            <div class="w-full h-full flex justify-center items-center flex-col relative">
                <div @click="resetAll" class="w-full h-full absolute z-30"></div>
                <div class="mx-auto w-4/12  rounded-xl shadow bg-white p-6 z-40 space-y-5">
                    <div class="text-xl text-red-500 font-semibold text-center">
                        <p>{{ $t('motdepaseoublie') }} </p>
                    </div>
                    <div class="text-center font-medium text-md">
                        <p>{{ $t('msgresetpassword') }}</p>
                    </div>
                    <div class="w-full flex justify-center">
                        <input type="email" v-model="email" placeholder="example@gmail.com*"
                            class="w-8/12 mx-auto focus:shadow duration-300 placeholder:text-sm outline-none py-2 px-3 rounded-md border border-[#94c12f]"
                            name="" id="">
                    </div>
                    <div class="w-full flex justify-center">
                        <button @click="sendMail" class="bg-[#94c12f] text-white px-3 py-1 rounded text-sm"
                            v-if="!loadEmail">{{ $t('recevoirlecode') }}</button>
                        <div v-else
        ...
        <div v-if="CodeTap" class="w-full h-full flex justify-center items-center  absolute backdrop-blur-md z-30">
            <div class="w-full h-full flex justify-center items-center flex-col relative">
                <div @click="resetAll" class="w-full h-full absolute z-30"></div>
                <div class="mx-auto w-4/12  rounded-xl shadow bg-white p-6 z-40 space-y-5">
                    <div class="text-xl text-red-500 font-semibold text-center">
                    ...
                    <div class="text-center font-medium text-md">
                        <p>{{ $t('si') }} <span class="text-lg font-medium text-[#94c12f]">{{ email }}</span> {{
                            $t('msgrecucode') }} </p>
                    </div>
                    <div class="w-full flex justify-center">
                        <input @change="confirmCode" v-model="inputCode" type="number" style=" -moz-appearance:textfield"
                            placeholder="51313*"
                            class="w-8/12 mx-auto focus:shadow duration-300 placeholder:text-sm outline-none py-2 px-3 rounded-md border focus:border-[#94c12f]"
                            name="" id="">
                    </div>

                    <div v-if="loadcode" class="w-full flex justify-center  items-center">
                        <div
                            class="animate-spin  ease-linear rounded-full w-10 h-10 border-t-2 border-b-2 border-blue-500 ml-3">
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div v-if="ResetTap" class="w-full h-full flex justify-center items-center  absolute backdrop-blur-md z-30">
            <div class="w-full h-full flex justify-center items-center flex-col relative">
                <div @click="resetAll" class="w-full h-full absolute z-30"></div>
                <div class="mx-auto w-4/12  rounded-xl shadow bg-white p-6 z-40 space-y-5">
                    <div class="text-xl text-red-500 font-semibold text-center">
                    ...
                    <div class="text-center font-medium text-md">
                        <p>{{ $t('saisirlesmotsdepasses') }}</p>
                    </div>
                    <div class="w-full flex justify-center">
                        <input type="password" placeholder="new password*" v-model="reset.password"
                            class="w-9/12 mx-auto focus:shadow duration-300 placeholder:text-sm outline-none py-2 px-3 rounded-md border focus:border-[#94c12f]"
                            name="" id="">
                    </div>
                    <div class="w-full flex justify-center">
                        <input type="password" @change="resetPassword" v-model="reset.pass_confirm"
                            placeholder="confirm new password*"
                            class="w-9/12 mx-auto focus:shadow duration-300 placeholder:text-sm outline-none py-2 px-3 rounded-md border focus:border-[#94c12f]"
                            name="" id="">
                    </div>

                    <div v-if="resetcode" class="w-full flex justify-center  items-center">
                        <div
                            class="animate-spin  ease-linear rounded-full w-10 h-10 border-t-2 border-b-2 border-blue-500 ml-3">
                        </div>
                    </div>
        ...
        <div v-if="LangueTap"
            class="w-full h-full overflow-hidden flex justify-center items-center  absolute backdrop-blur-md z-30">
            <div class="w-full h-full flex justify-center items-center flex-col relative">
                <div @click="resetAll" class="w-full h-full absolute z-30"></div>
                <div class="mx-auto w-10/12 md:w-4/12  rounded-xl shadow bg-white p-6 z-40 space-y-5">
                    <div class="text-xl text-red-500 font-semibold text-center">
                        <p>{{ $t('changervotrelangueactuelle') }}</p>
                    </div>
                    <div class="text-center font-medium text-md">
                        <p>
                            {{ $t('pharsechangelangue') }}
                        </p>
                    </div>
                    <div class="w-full flex justify-center space-x-3">
                        <div @click="changeLangue('fr')" v-if="currentLangue != 'fr'"
                            class="w-20 cursor-pointer h-14 rounded border border-[#94c12f] overflow-hidden p-2">
                            <img src="/img/icons/france-icon.png" class="w-full h-full object-cover object-center" alt=""
                                srcset="">
                        </div>
                        <div @click="changeLangue('en')" v-if="currentLangue != 'en'"
                            class="w-20 cursor-pointer h-14 rounded border border-[#94c12f] overflow-hidden p-2">
                            <img src="/img/icons/usa-icon.png" class="w-full h-full object-cover object-center" alt=""
                                srcset="">
                        </div>
                        <div @click="changeLangue('de')" v-if="currentLangue != 'de'"
                            class="w-20 cursor-pointer h-14 rounded border border-[#94c12f] overflow-hidden p-2">
                            <img src="/img/icons/german-icon.png" class="w-full h-full object-cover object-center" alt=""
                                srcset="">
                        </div>
                    </div>
                    <div class="w-full flex justify-center">
        ...
        <div @click="LangueTap = true"
            class="  w-auto  md:max-w-[35%]  max-w-[80%] space-x-2  font-medium flex justify-center items-center   absolute z-40 right-9 top-3 ">
            <div v-if="currentLangue == 'fr'" class="w-12 space-x-2 h-8 flex text-white items-center">
                <img src="/img/icons/france-icon.png" class="h-full object-cover object-center cursor-pointer" alt=""
                    srcset="">
                <span class="text-black font-semibold">fr</span>
            </div>

            <div v-if="currentLangue == 'de'" class="w-12 space-x-2 h-8 flex text-white items-center">
                <img src="/img/icons/german-icon.png" class="h-full object-cover object-center cursor-pointer" alt=""
                    srcset="">
                <span class="text-black font-semibold">de</span>
            </div>

            <div v-if="currentLangue == 'en'" class="w-12 space-x-2 h-8 flex text-white items-center">
                <img src="/img/icons/usa-icon.png" class="h-full object-cover object-center cursor-pointer" alt=""
                    srcset="">
                <span class="text-black font-semibold">en</span>
            </div>

        </div>
        <div
            class="w-full absolute lg:relative lg:w-1/2 z-10 h-full bg-white bg-opacity-60 sm:bg-opacity-90 flex justify-center items-center">
            <div class="w-10/12 md:w-7/12 lg:w-6/12 space-y-3 ">
                <div class="text-center text-3xl font-nunito ">
                    <span>{{ $t('bienvenue') }} <span class="text-[#94C12F]">S</span><span id="last"
                            class="text-black">CHMID</span></span>
                </div>
                <div class="w-full  space-y-1">
                    <div class="w-full px-5">
                        <label for="email" class="text-lg  text-[#94C12F]  font-nunito capitalize">{{ $t('email') }}</label>
                    </div>
                    <div class="w-full">
                        <input type="email" @input="updateText('email')" v-model="form.email"
                            placeholder="Example@gmail.com*"
                            :class="AlertErrorEmail ? 'border-red-400 border duration-300' : 'border-[#94C12F] border'"
                            class="outline-none text-sm placeholder:text-gray-400 placeholder:text-md font-medium p-3 w-full rounded-2xl "
                            name="" id="">
                    </div>
                </div>
                <div class="w-full  space-y-1">
                    <div class="w-full px-5">
                        <label for="password" class="text-lg  text-[#94C12F]  font-nunito capitalize">{{ $t('motdepasse')
                        }}</label>
                    </div>
                    <div :class="AlertErrorPassword ? 'border-red-400 border duration-300' : 'border-[#94C12F] border'"
                        class="w-full h-12 rounded-2xl  flex overflow-hidden ">
                        <div class="w-10/12  h-full">
                            <input :type="showPass ? 'text' : 'password'" @input="updateText('password')"
                                v-model="form.password" placeholder="*********"
                                class="outline-none text-sm placeholder:text-gray-400 placeholder:text-md font-medium p-3 w-full  "
                                name="" id="">
                        </div>
                        <div class="w-2/12 bg-white h-full flex justify-center items-center">
                            <svg v-if="!showPass" @click="showPass = !showPass" xmlns="http://www.w3.org/2000/svg"
                                fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
                                class="w-6 h-6 cursor-pointer text-[#94C12F]">
                                <path stroke-linecap="round" stroke-linejoin="round"
                                    d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z" />
                                <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                            </svg>
                            <svg v-else @click="showPass = !showPass" xmlns="http://www.w3.org/2000/svg" fill="none"
                                viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 text-[#94C12F]">
                                <path stroke-linecap="round" stroke-linejoin="round"
                ...
                <div class="w-full flex justify-center items-center">
                    <button @click="login"
                        class=" mx-auto sm:w-6/12 lg:w-full py-3 w-full bg-[#94C12F] text-white font-nunito text-center rounded-2xl">
                        {{ $t('btnlogin') }}
                    </button>
                </div>

                <!-- <div class="flex justify-between items-center w-full">
                    <hr class="border-spacing-y-24 w-1/3">
                ...
                <!-- <div class="justify-center space-x-5 flex items-center ">
                    <div
                        class="h-14 md:h-16 cursor-pointer w-14 md:w-16 rounded-2xl border flex justify-center items-center">
                        <img src="/img/icons/face.webp" class="w-full h-full " alt="" srcset="">
                    </div>
                    <div
                        class="h-14 md:h-16 cursor-pointer w-14 md:w-16 rounded-2xl border flex justify-center items-center">
                        <img src="/img/icons/google.png" class="w-8/12 h-4/6 " alt="" srcset="">
                    </div>
                </div> -->

                <div class="flex w-full justify-between pt-6 items-center">
                    <div @click="MailTap = true"
                        class=" cursor-pointer capitalize  text-[#94C12F] text-xs sm:text-lg lg:text-sm">
                        <p>{{ $t('motdepaseoublie') }}</p>
                    </div>
        ...
        <div class="w-full flex justify-center items-center z-0 lg:w-1/2 h-full  md:relative">
            <svg class="z-0 " version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
		...
		C56.328,303.153,56.328,356.401,125.235,389.243z" />
                ...
                <g id="Shadow">
                    <path id="XMLID_418_" style="fill:#EBEBEB;"
                        d="M358.282,361.766c8.974,4.277,23.522,4.277,32.496,0
		c8.974-4.276,8.974-11.211,0-15.488c-8.974-4.276-23.522-4.276-32.496,0C349.309,350.555,349.309,357.489,358.282,361.766z" />
                ...
                <g id="Plants">
                    <g>
                        <g id="XMLID_292_">
                            <g id="XMLID_378_">
                                <g>
                                    <path id="XMLID_372_" style="fill:#94C12F;"
                                        d="M119.954,355.593l8.124-4.69c0,0,6.702-4.926,10.021-14.071
						c2.511-6.92,2.165-12.185,0.364-14.823c-2.215-3.247-7.946-2.68-8.174,4.166C129.841,339.654,125.908,348.98,119.954,355.593z" />
                                    <path id="XMLID_1_" style="opacity:0.2;"
                                        d="M119.954,355.593l8.124-4.69c0,0,6.702-4.926,10.021-14.071
						c2.511-6.92,2.165-12.185,0.364-14.823c-2.215-3.247-7.946-2.68-8.174,4.166C129.841,339.654,125.908,348.98,119.954,355.593z" />
                                </g>
					...
					c0.041,0.188,3.839,18.943-15.525,32.343C120.152,356.065,120.053,356.093,119.955,356.093z" />
                            ...
                            <g id="XMLID_379_">
                                <path id="XMLID_364_" style="fill:#94C12F;" d="M103.706,364.974c0,0-2.828-2.303-2.144-5.42
					...
					c-0.154,0.299-3.85,7.36-10.032,9.837C112.796,354.865,112.733,354.877,112.671,354.877z" />
                        ...
                        <g>
                            <g id="XMLID_366_">
                                <path id="XMLID_374_" style="fill:#94C12F;" d="M316.802,301.083c0,0-4.373-7.072-4.795-14.772
					...
					c-0.106,0.169-2.704,4.148-10.34,6.078C323.935,289.166,323.894,289.171,323.853,289.171z" />
                            ...
                            <g>
                                <g>
                                    <path id="XMLID_363_" style="fill:#94C12F;" d="M208.022,285.535c0,0-9.589-2.338-10.088-20.293
					...
					C208.469,298.406,208.258,298.575,208.021,298.575z" />
                                <g id="XMLID_380_">
                                    <path id="XMLID_361_" style="fill:#94C12F;"
                                        d="M208.022,344.67c-9.915,1.344-26.019,1.32-35.295-5.996
				...
				S165.57,154.463,161.016,158.044z" />
                            <polygon id="XMLID_271_" style="fill:#F5F5F5;"
                                points="168.102,124.043 174.586,127.782 170.925,134.76 164.439,131.02 			" />
                            <polygon id="XMLID_144_" style="fill:#F5F5F5;"
                                points="184.56,134.999 178.997,138.66 172.511,134.92 178.075,131.259 			" />
                            <polygon id="XMLID_143_" style="fill:#E0E0E0;"
                                points="183.666,152.833 179.455,151.036 172.97,147.296 177.181,149.093 			" />
                            <polygon id="XMLID_621_" style="fill:#E0E0E0;"
                                points="145.99,173.615 152.476,177.355 155.138,170.154 148.653,166.414 			" />
                            <polygon id="XMLID_142_" style="fill:#E0E0E0;"
                                points="166.494,175.503 160.008,171.763 158.424,166.504 164.909,170.244 			" />
				...
				c4.554-3.581,8.913-2.606,9.737,2.177C175.079,151.423,172.055,158.203,167.501,161.784z" />
                            <polygon id="XMLID_603_" style="fill:#F5F5F5;"
                                points="165.525,130.743 159.04,127.003 153.104,131.67 159.589,135.41 			" />
                            <polygon id="XMLID_614_" style="fill:#E0E0E0;"
                                points="153.104,131.67 153.493,137.87 159.977,141.609 159.589,135.41 			" />
                            <polygon id="XMLID_615_" style="fill:#F5F5F5;"
                                points="156.696,145.947 150.21,142.207 145.306,141.965 151.791,145.705 			" />
                            <polygon id="XMLID_616_" style="fill:#E0E0E0;"
                                points="145.306,141.965 141.867,149.673 148.352,153.413 151.791,145.705 			" />
                            <polygon id="XMLID_617_" style="fill:#F5F5F5;"
                                points="151.732,160.095 145.247,156.355 139.9,161.272 146.385,165.012 			" />
                            <polygon id="XMLID_619_" style="fill:#E0E0E0;"
                                points="147.459,171.247 140.974,167.507 139.9,161.272 146.385,165.012 			" />
                            <polygon id="XMLID_145_" style="fill:#F5F5F5;"
                                points="174.586,127.782 168.102,124.043 173.058,125.151 179.543,128.891 			" />
                        </g>
                        <g id="XMLID_558_">
				...
				C135.365,120.206,130.349,128.893,124.162,132.465z" />
                            <polygon id="XMLID_582_" style="fill:#F5F5F5;"
                                points="146.132,89.865 139.646,86.125 133.828,94.877 140.313,98.616 			" />
                            <polygon id="XMLID_581_" style="fill:#F5F5F5;"
                                points="156.578,103.642 150.093,99.902 142.682,103.243 149.167,106.983 			" />
                            <polygon id="XMLID_580_" style="fill:#E0E0E0;"
                                points="126.615,155.54 120.13,151.8 119.317,143.711 125.803,147.451 			" />
                            <polygon id="XMLID_576_" style="fill:#E0E0E0;"
                                points="102.975,149.642 109.46,153.382 114.129,143.967 107.644,140.227 			" />
				...
				C141.85,123.946,136.834,132.633,130.647,136.205z" />
                            <polygon id="XMLID_559_" style="fill:#F0F0F0;"
                                points="114.497,106.037 120.982,109.777 115.163,107.745 108.678,104.005 			" />

                            <rect id="XMLID_563_" x="103.366" y="107.07"
                                transform="matrix(0.4996 -0.8662 0.8662 0.4996 -41.4157 149.9286)" style="fill:#F5F5F5;"
                                width="11.405" height="7.486" />
                            <polygon id="XMLID_564_" style="fill:#E0E0E0;"
                                points="102.975,113.882 109.46,117.622 114.129,121.646 107.644,117.905 			" />
                            <polygon id="XMLID_574_" style="fill:#F5F5F5;"
                                points="105.643,124.628 112.128,128.368 104.717,133.585 98.231,129.845 			" />
                            <polygon id="XMLID_575_" style="fill:#E0E0E0;"
                                points="98.231,129.845 104.717,133.585 104.717,142.896 98.231,139.157 			" />
                            <polygon id="XMLID_602_" style="fill:#F5F5F5;"
                                points="134.68,90.999 128.194,87.259 120.13,91.915 126.615,95.655 			" />
                        </g>
                    </g>
                </g>
                <g id="Device">
			...
			C224.269,172.777,225.433,170.762,226.868,169.934z" />
                        <path id="XMLID_51_" style="fill:#37474F;"
                            d="M266.921,126.294c-0.841,0.486-1.523,1.667-1.523,2.638
			c0,0.972,0.683,1.365,1.523,0.879c0.842-0.485,1.523-1.666,1.523-2.638C268.444,126.202,267.763,125.808,266.921,126.294z" />
                        <path id="XMLID_50_" style="fill:#37474F;"
                            d="M266.923,308.738c-3.322,1.918-6.015,6.582-6.015,10.418
			c0,3.835,2.692,5.39,6.015,3.472c3.321-1.917,6.014-6.581,6.014-10.417C272.936,308.376,270.244,306.821,266.923,308.738z" />
                        <g id="XMLID_47_">
                            <path id="XMLID_49_" style="fill:#37474F;"
                                d="M252.533,141.771c-1.682,0.972-3.046,3.334-3.046,5.277
			...
			c0,1.554-1.091,2.184-2.438,1.407c-1.346-0.777-2.437-2.667-2.437-4.222S212.665,194.748,214.01,195.525z" />
                        <g id="XMLID_294_">
                            <path id="XMLID_43_" style="fill:#FFFFFF;stroke:#94C12F;stroke-miterlimit:10;" d="M325.821,129.889
				...
				c0,1.657,1.163,2.328,2.598,1.5l70.543-40.729l9.563-5.521c1.435-0.829,2.599-2.844,2.599-4.5L325.821,129.889z" />
                            <path style="fill:#94C12F;stroke:#94C12F;stroke-miterlimit:10;" d="M241.956,330.279c-0.333,0-0.646-0.08-0.926-0.241
				...
				l70.543-40.729l0.5,0.865l-70.543,40.728C242.879,330.138,242.399,330.279,241.956,330.279z" />
                            <path style="fill:#94C12F;stroke:#94C12F;stroke-miterlimit:10;" d="M313.91,289.127l-0.5-0.865l9.563-5.52
				...
				c0,1.843-1.251,4.01-2.849,4.933L313.91,289.127z" />
                        ...
                        <g>
                            <path id="XMLID_42_" style="fill:#94C12F;"
                                d="M283.17,164.464c8.974-5.181,16.248-0.981,16.248,9.381
				s-7.274,22.961-16.248,28.143c-8.974,5.181-16.248,0.981-16.248-9.381C266.922,182.245,274.196,169.645,283.17,164.464z" />
                            <path id="XMLID_41_" style="fill:#94C12F;"
                                d="M283.17,172.619c2.911-1.681,5.271-0.318,5.271,3.043
				c0,3.362-2.36,7.45-5.271,9.13c-2.911,1.681-5.272,0.318-5.272-3.044C277.898,178.387,280.259,174.3,283.17,172.619z" />
                        </g>
                        <path id="XMLID_274_" style="fill:#94C12F;"
                            d="M293.444,190.544c0-5.523-3.878-7.762-8.66-5l-2.99,1.726
			c-4.782,2.761-8.66,9.477-8.66,15v1.486c2.765,0.918,6.245,0.421,10.036-1.769c3.898-2.251,7.475-5.904,10.274-10.163V190.544z" />
                        <g>
                            <path id="XMLID_5_" style="opacity:0.7;fill:#FFFFFF;"
                                d="M283.17,172.619c2.911-1.681,5.271-0.318,5.271,3.043
				c0,3.362-2.36,7.45-5.271,9.13c-2.911,1.681-5.272,0.318-5.272-3.044C277.898,178.387,280.259,174.3,283.17,172.619z" />
                            <path id="XMLID_4_" style="opacity:0.7;fill:#FFFFFF;"
                                d="M293.444,190.544c0-5.523-3.878-7.762-8.66-5l-2.99,1.726
				c-4.782,2.761-8.66,9.477-8.66,15v1.486c2.765,0.918,6.245,0.421,10.036-1.769c3.898-2.251,7.475-5.904,10.274-10.163V190.544z" />
                        </g>
                        <g>
                            <path id="XMLID_482_" style="fill:#94C12F;" d="M251.731,224.839l20.908-12.071c0.957-0.552,1.733-0.104,1.733,1v0.69
				...
				C249.999,226.734,250.775,225.391,251.731,224.839z" />
                        ...
                        <g>
                            <path id="XMLID_483_" style="fill:#94C12F;" d="M250.866,239.41l63.26-36.523c0.478-0.276,0.866-0.053,0.866,0.5v7.381
				...
				C250,240.358,250.388,239.686,250.866,239.41z" />
                        ...
                        <g>
                            <path id="XMLID_512_" style="fill:#94C12F;" d="M251.731,257.672l20.908-12.071c0.957-0.552,1.733-0.105,1.733,1v0.69
				...
				C249.999,259.567,250.775,258.225,251.731,257.672z" />
                        ...
                        <g>
                            <path id="XMLID_511_" style="fill:#94C12F;" d="M250.866,272.243l63.26-36.523c0.478-0.276,0.866-0.052,0.866,0.5v7.381
				...
				C250,273.191,250.388,272.52,250.866,272.243z" />
                        ...
                        <g>
                            <path id="XMLID_510_" style="fill:#94C12F;" d="M276.104,276.434l24.97-14.416c0.957-0.553,1.732-0.105,1.732,1v7.726
			...
			c2.146-4.481,1.712-11.043-1.482-12.948C369.962,195.98,365.155,197.334,363.681,198.295z" />
                        <g>
                            <path id="XMLID_283_" style="fill:#94C12F;" d="M382.148,250.824c3.084,4.799,6.016,9.026,6.641,14.419
				...
				s-4.083,1.244-6.146-0.543l-3.858-60.434l2.032-7.357L382.148,250.824z" />
                        ...
                        <g>
                            <path id="XMLID_40_" style="fill:#94C12F;" d="M372.145,281.659l2.032-7.357c0,0,5.634-1.361,11.013-4.368
				...
				c0,0-2.085,3.786-8.056,5.862l-3.11,9.458l-0.268,21.644L372.145,281.659z" />
                        ...
                        <g>
                            <path id="XMLID_281_" style="fill:#94C12F;" d="M382.148,250.824c1.65,2.693,5.594,7.298,4.756,11.723
				...
				c0.516-3.882,6.038-46.352,6.038-46.352L382.148,250.824z" />
                        ...
                        <g>
                            <path id="XMLID_286_" style="fill:#94C12F;"
                                d="M385.19,228.017l-20.6-3.326c-0.422-0.092-1.103-0.263-1.963-0.49l-0.002-0.001
				...
				s7.99,3.872,14.568,1.964c6.579-1.908,8.785-4.657,8.785-4.657l-3.291-5.363C379.754,243.246,383.892,234.292,385.19,228.017z" />
                            <path id="XMLID_15_" style="opacity:0.7;fill:#FFFFFF;"
                                d="M385.19,228.017l-20.6-3.326c-0.422-0.092-1.103-0.263-1.963-0.49
				...
				s7.99,3.872,14.568,1.964c6.579-1.908,8.785-4.657,8.785-4.657l-3.291-5.363C379.754,243.246,383.892,234.292,385.19,228.017z" />
                        ...
                        <path id="XMLID_543_" style="fill:#94C12F;" d="M379.756,243.351c-0.084,0.194-6.622,3.697-8.565,4.233
...
export default {
    ...
    mounted() {
        ...
        this.currentLangue = localStorage.getItem("langue");
        this.$i18n.locale = this.currentLangue;
    },
    data() {
        return {
            form: {
    ...
    methods: {
        ...
        resetAll() {
            ...
            this.form.password = ''
            this.reset.password = ''
            this.reset.pass_confirm = ''
            this.code = ''
            this.email = ''
            this.inputCode = ''
            this.AlertErrorEmail = false
            this.AlertErrorPassword = false
            this.loading = false
            this.showPass = false
        ...
        login() {
            ...
            if (!this.form.email.includes('@') || !this.form.email.includes('.')) {
                this.alertError('Erreur d\'email', ' l\' email n\'est pas valide')
                this.AlertErrorEmail = true
            } else if (this.form.password.length < 8 || !name.test(this.form.password)) {
                this.alertError('Erreur du mot de passe', 'mot de passe doit contenir un caratere majuscule et avoir plus de 8 carateres')
                this.AlertErrorPassword = true
            } else {
                this.axios.post(this.api + 'login', this.form).then(({ data }) => {
                    console.log(data);
                    localStorage.setItem('jwtToken', data.access_token.token)
                    this.$router.push('/')
                }).catch(error => {
                    this.alertError('erreur',
                        error.response.data.errors[0] == 'Password mismatch' ? this.$i18n.messages[`${localStorage.getItem('langue')}`].motdepasseincorret
                            : error.response.data.errors[0] == 'User does not exist' ? this.$i18n.messages[`${localStorage.getItem('langue')}`].usernotfound
                                : error.response.data.errors[0] == 'The email field is required' ? this.$i18n.messages[`${localStorage.getItem('langue')}`].emailrequired
                                    : this.$i18n.messages[`${localStorage.getItem('langue')}`].passwordrequired
                    )
        ...
        sendMail() {
            ...
            if (!this.email.includes(".") && !this.email.includes("@")) {
                this.alertError('Erreur mail recuperation', 'votre email n\'est pas valide pour la recuperation')
            } else {
                this.loadEmail = true
                let data = new FormData();
                data.append('email', this.email)
                this.axios.post(this.api + 'send-mail', data).then(({ data }) => {
                    this.loadEmail = false
                    ...
                    this.alertError('Erreur mail recuperation', error.response.data.message)
                });
            }
        ...
        confirmCode() {
            if (this.code == this.inputCode) {
                ...
                this.alertError('Erreur code', 'le code saisi ne correspond, reverifiez...')
            }
        },
        ...
        resetPassword() {
            ...
            if (this.reset.password.length < 8) {
                this.alertError('Erreur password', 'le mot de passe saisi doit avoir au moins 8 carateres')
            } else if (!name.test(this.reset.password)) {
                this.alertError('Erreur password', 'le mot de passe saisi doit debuter avec un caratere majuscule')
            } else if (this.reset.password != this.reset.pass_confirm) {
                this.alertError('Erreur password', 'les mots de passe ne correspondent pas')
            } else {
                let data = new FormData();
                data.append('password', this.reset.password)
                data.append('_method', 'put')
                this.axios.post(this.api + 'resetpassword/' + JSON.parse(localStorage.getItem('user')).id, data).then(({ data }) => {
                    this.resetcode = false
                    this.ResetTap = false
                    console.log(data);
                    localStorage.removeItem('user');
                    this.alertValide('confirmation de mise a jour', 'mot de passe modife avec success')
                }).catch(error => {
                    this.resetcode = false
                    this.alertError('Erreur mail recuperation', error)
                });
            }
