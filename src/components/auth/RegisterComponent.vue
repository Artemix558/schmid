<template>
  <div class="w-screen h-[90vh] md:h-screen overflow-hidden bg-white md:flex relative">
    ...
    <div v-if="LangueTap"
      ...
      <div class="w-full h-full flex justify-center items-center flex-col relative">
        <div @click="LangueTap = !LangueTap" class="w-full h-full absolute z-30"></div>
        <div class="mx-auto w-10/12 md:w-4/12  rounded-xl shadow bg-white p-6 z-40 space-y-5">
          ...
          <div class="w-full flex justify-center space-x-3">
            <div @click="changeLangue('fr')" v-if="currentLangue != 'fr'"
              ...
              <img src="/img/icons/france-icon.png" class="w-full h-full object-cover object-center" alt="" srcset="">
            </div>
            <div @click="changeLangue('en')" v-if="currentLangue != 'en'"
              class="w-20 cursor-pointer h-14 rounded border border-[#94c12f] overflow-hidden p-2">
              <img src="/img/icons/usa-icon.png" class="w-full h-full object-cover object-center" alt="" srcset="">
            </div>
            <div @click="changeLangue('de')" v-if="currentLangue != 'de'"
              class="w-20 cursor-pointer h-14 rounded border border-[#94c12f] overflow-hidden p-2">
              <img src="/img/icons/german-icon.png" class="w-full h-full object-cover object-center" alt="" srcset="">
            </div>
          </div>
    ...
    <div @click="LangueTap = true"
      class="  w-auto  md:max-w-[35%]  max-w-[80%] space-x-2  font-medium flex justify-center items-center   absolute z-30 right-9 top-3 ">
      <div v-if="currentLangue == 'fr'" class="w-12 space-x-2 h-8 flex text-white items-center">
        <img src="/img/icons/france-icon.png" class="h-full object-cover object-center cursor-pointer" alt="" srcset="">
      ...
      <div v-if="currentLangue == 'de'" class="w-12 space-x-2 h-8 flex text-white items-center">
        <img src="/img/icons/german-icon.png" class="h-full object-cover object-center cursor-pointer" alt="" srcset="">
      ...
      <div v-if="currentLangue == 'en'" class="w-12 space-x-2 h-8 flex text-white items-center">
        <img src="/img/icons/usa-icon.png" class="h-full object-cover object-center cursor-pointer" alt="" srcset="">
        <span class="text-black font-semibold">en</span>
      </div>

    </div>
    <div
      class="w-full absolute lg:relative lg:w-1/2 z-10 h-full bg-white bg-opacity-60 sm:bg-opacity-100 flex justify-center items-center">
      <div class="w-10/12 md:w-7/12 lg:w-6/12 space-y-3 ">
        ...
        <div class="w-full  space-y-1">
          <div class="w-full px-5">
            <label for="name" class="text-lg  text-[#94C12F]  font-nunito capitalize">{{ $t('nom') }}</label>
          </div>
          <div class="w-full">
            <input type="text" @input="updateText('name')" v-model="form.name" placeholder="Example@gmail.com*"
              :class="AlertErrorName ? 'border-red-400 border duration-300' : 'border-[#94C12F] border'"
        ...
        <div class="w-full  space-y-1">
          ...
          <div class="w-full">
            <input type="email" @input="updateText('email')" v-model="form.email" placeholder="Example@gmail.com*"
        ...
        <div class="w-full  space-y-1">
          <div class="w-full px-5">
            <label for="password" class="text-lg  text-[#94C12F]  font-nunito capitalize">{{ $t('motdepasse') }}</label>
          </div>
          <div :class="AlertErrorPassword ? 'border-red-400 border duration-300' : 'border-[#94C12F] border'"
            ...
            <div class="w-10/12  h-full">
              <input :type="showPass ? 'text' : 'password'" @input="updateText('password')" v-model="form.password"
                placeholder="*********"
            ...
            <div class="w-2/12 bg-white h-full flex justify-center items-center">
              <svg v-if="!showPass" @click="showPass = !showPass" xmlns="http://www.w3.org/2000/svg" fill="none"
              ...
              <svg v-else @click="showPass = !showPass" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
        ...
        <div class="flex w-full justify-center pt-6 items-center">

          <router-link to="/Login" class=" cursor-pointer capitalize text-[#94C12F] text-xs sm:text-lg lg:text-sm">
            <p> <span class="text-black">{{ $t('jaidejauncompte') }},</span> {{ $t('senregistrer') }}</p>
          </router-link>
        </div>
      </div>
    </div>
          ...
          <g id="XMLID_272_">
            ...
            <g id="XMLID_558_">
              ...
              <rect id="XMLID_563_" x="103.366" y="107.07"
                transform="matrix(0.4996 -0.8662 0.8662 0.4996 -41.4157 149.9286)" style="fill:#F5F5F5;" width="11.405"
                height="7.486" />
...
export default {
  ...
  methods: {
    register() {
      var name = /^([A-Z])/
      if ((this.form.name.length < 5 || !name.test(this.form.name))) {
        this.alertError('Erreur sur le nom', 'votre nom doit etre debuter par une lettre majuscule et doit avoir plus de 5 carateres')
        ...
        this.axios.post(this.api + 'register', this.form).then(({ data }) => {
          console.log(data);
