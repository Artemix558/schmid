<template>
    <div>
        <div class="bg-[#171717] w-screen h-screen flex flex-col justify-center items-end relative">
            <div v-if="modalTap"
                class="bg-[#94C12F] w-full  h-full absolute bg-opacity-20 backdrop-blur flex justify-center items-center"
                style="z-index: 101;">
                <div class="w-full h-full relative flex justify-center items-center">
                    <div @click="modalTap = !modalTap" class="w-full h-full absolute"></div>
                    <div style="z-index: 102;"
                        class="w-full md:w-5/12 py-9 rounded-xl bg-white flex justify-center items-center">
                        <div class=" w-9/12 space-y-4">
                            <div
                                class="mx-auto p-3 rounded text-white font-semibold flex justify-center space-x-3 bg-[#94C12F] ">
                                <div>
                                ...
                                <div class="uppercase">
                                    <span>{{ type == 'add' ? 'ajout' : 'suppression' }} d'element a transporter</span>
                                </div>
                            </div>

                            <div class="text-lg font-nunito text-center">
                                <p>Vous souhaitez {{ type == 'add' ? 'ajoutez' : 'retirez' }} <span
                                        class="text-[#94C12F] font-semibold">{{ item.name }}</span>
                                    dans votre liste de lot a demenager</p>
                            </div>

                            <div class="w-8/12 mx-auto">
                                <input v-if="type == 'add'" style=" -moz-appearance:textfield" type="number"
                                    v-model="item.occ" class="w-full px-2 py-1 rounded border outline-none border-[#94C12F]"
                                    name="" id="">
                            </div>
                            <div class="w-full flex justify-center ">
                                <button @click="SaveItem" v-if="!loadModal && type == 'add'"
                                    class="px-5 py-2 mx-auto bg-[#94C12F] text-white rounded font-nunito">
                                    validez
                                </button>
                                <button @click="LeaveItem" v-if="!loadModal && type == 'remove'"
                                    class="px-5 py-2 mx-auto bg-[#94C12F] text-white rounded font-nunito">
                                    retirez
                                </button>
                                <button @click="modalTap = false" v-if="!loadModal"
                                    class="px-5 py-2 mx-auto bg-red-500 text-white rounded font-nunito">
                                    annuler
                                </button>
                                <div v-else
                                    class="animate-spin ease-linear rounded-full w-10 h-10 border-t-2 border-b-2 border-[#94C12F] ml-3">
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div v-if="modalFragileTap"
                class="bg-[#94C12F] w-full  h-full absolute bg-opacity-20 backdrop-blur flex justify-center items-center"
                style="z-index: 101;">
                <div class="w-full h-full relative flex justify-center items-center">
                    <div @click="modalFragileTap = !modalFragileTap" class="w-full h-full absolute"></div>
                    <div style="z-index: 102;"
                        class="w-full md:w-5/12 py-9 rounded-xl bg-white flex justify-center items-center">
                        <div class=" w-11/12 space-y-4">
                            <div
                                ...
                                <div class="uppercase text-center">
                                    <span>Ajout d'element a transporter avec precautions</span>
                                </div>
                            </div>

                            <div class="text-lg font-nunito text-center">
                                <p>Veuillez renseignent l'element et sa quantite</p>
                            </div>

                            <div class="w-6/12 mx-auto">
                                <input type="text" v-model="item.name" placeholder="ampoule*"
                                    class="w-full px-3 py-2 rounded-md text-sm border outline-none border-[#94C12F]" name=""
                                    id="">
                            </div>
                            <div class="w-6/12 mx-auto">
                                <input style=" -moz-appearance:textfield" type="number" v-model="item.occ"
                                    class="w-full px-3 py-2 rounded-md text-sm border outline-none border-[#94C12F]" name=""
                                    id="">
                            </div>
                            <div class="w-full flex justify-center ">
                                <button @click="addFragileItem" v-if="!loadFragileModal"
                                    class="px-5 py-2 mx-auto bg-[#94C12F] text-white rounded font-nunito">
                                    validez
                                </button>
                                <button @click="modalFragileTap = !modalFragileTap" v-if="!loadFragileModal"
                                    class="px-5 py-2 mx-auto bg-red-500 text-white rounded font-nunito">
                                    annuler
                                </button>
                                <div v-else
            ...
            <div v-if="modal"
                ...
                style="z-index: 101;">
                <div class="w-full h-full relative flex justify-center items-center">
                    <div @click="modal = !modal" class="w-full h-full absolute"></div>
                    <div style="z-index: 102;" class="w-7/12 py-9 rounded-xl bg-white flex justify-center items-center">
                        <div class=" w-11/12 space-y-2">
                            <div
                                ...
                                <div class="uppercase text-center">
                                    <span>finalisons l'enrgistrement</span>
                                </div>
                            </div>

                            <div class="text-lg font-nunito text-center">
                                <p>Quel est la position de local</p>
                            </div>

                            <div class="w-10/12 mx-auto font-semibold text-lg">
                                <p>Local actuel(depart)</p>
                            </div>
                            <div class="w-8/12 mx-auto grid grid-cols-2 gap-3">
                                <div>
                                    <label for="departure">liaison de deplacement</label>
                                    <select v-model="form.liaison_local_depart"
                                        class="w-full px-3 py-2 rounded-md text-sm border outline-none border-[#94C12F]"
                                        name="" id="">
                                        <option value="a l'etage">ascenseur</option>
                                        <option value="au rez-de-chaussee">escalier</option>
                                        <option value="au sous-sol">escalier/ascenseur</option>
                                    </select>

                                </div>

                                <div>
                                    <label for="departure">local d'arrivee</label>
                                    <select v-model="form.situation_local_depart"
                                        class="w-full px-3 py-2 rounded-md text-sm border outline-none border-[#94C12F]"
                                        name="" id="">
                                        <option value="a l'etage">a l'etage</option>
                                        <option value="au rez-de-chaussee">au rez-de-chaussee</option>
                                        <option value="au sous-sol">au sous-sol</option>
                                    </select>
                                </div>
                            </div>

                            <div class="w-10/12 mx-auto font-semibold text-lg">
                                <p>Local d'arrivee(destination)</p>
                            </div>
                            <div class="w-8/12 mx-auto grid grid-cols-2 gap-3">
                                <div>
                                    <label for="departure">liaison de deplacement</label>
                                    <select v-model="form.liaison_local_arrivee"
                                        class="w-full px-3 py-2 rounded-md text-sm border outline-none border-[#94C12F]"
                                        name="" id="">
                                        <option value="a l'etage">ascenseur</option>
                                        <option value="au rez-de-chaussee">escalier</option>
                                ...
                                <div>
                                    <label for="departure">local d'arrivee</label>
                                    <select v-model="form.situation_local_arrivee"
                                        class="w-full px-3 py-2 rounded-md text-sm border outline-none border-[#94C12F]"
                                        name="" id="">
                                        <option value="a l'etage">a l'etage</option>
                                        <option value="au rez-de-chaussee">au rez-de-chaussee</option>
                            ...
                            <div class="w-full flex justify-center ">
                                <button @click="SaveDem" v-if="!loadFragileModal"
                                    class="px-5 py-2 mx-auto bg-[#94C12F] text-white rounded font-nunito">
                                    validez
                                </button>
                                <button @click="modal = !modal" v-if="!loadFragileModal"
                                    class="px-5 py-2 mx-auto bg-red-500 text-white rounded font-nunito">
                                    annuler
                                </button>
                                <div v-else
            ...
            <div v-if="modalOtherTap"
                class="bg-[#94C12F] w-full  h-full absolute bg-opacity-20 backdrop-blur flex justify-center items-center"
                style="z-index: 101;">
                <div class="w-full h-full relative flex justify-center items-center">
                    <div @click="modalOtherTap = !modalOtherTap" class="w-full h-full absolute"></div>
                    <div style="z-index: 102;"
                        class="w-full md:w-5/12 py-9 rounded-xl bg-white flex justify-center items-center">
                        <div class=" w-11/12 space-y-4">
                            <div
                                ...
                                <div class="uppercase text-center">
                                    <span>Ajoutez des elements autres de votre habitations</span>
                                </div>
                            </div>
                            ...
                            <div class="w-full flex justify-center ">
                                <button @click="addotherItem" v-if="!loadOtherModal"
                                    class="px-5 py-2 mx-auto bg-[#94C12F] text-white rounded font-nunito">
                                    validez
                                </button>
                                <button @click="modalOtherTap = !modalOtherTap" v-if="!loadOtherModal"
                                    class="px-5 py-2 mx-auto bg-red-500 text-white rounded font-nunito">
                                    annuler
                                </button>
                                <div v-else
            ...
            <div v-if="loadScreen"
                class=" w-full  h-full absolute bg-opacity-20 backdrop-blur flex justify-center items-center"
                style="z-index: 101;">
                <div class="w-full h-full relative flex flex-col justify-center items-center">
                    ...
                    <div class="w-full py-3 uppercase text-center font-bold text-white">
                        {{ $t('encoursdechargement') }}...
                    </div>
                </div>
            </div>
            <div class="mx-auto w-10/12 h-[80%] rounded-lg  overflow-hidden">
                <div class="h-16">

                </div>
                <div id="steper" class="w-screen py-8   flex flex-col items-end justify-end ">
                    <div class="w-screen overflow-hidden overflow-x-scroll  flex space-x-2 ">
                        <div :class="{ 'text-white bg-[#94C12F] duration-500': step > 0, 'text-[#555555] bg-[#363636] duration-500': step == 0 }"
                            class="text-white  bg-[#232323] font-medium py-2 px-5 text-sm rounded-lg cursor-pointer ">
                            <span>1. {{ $t('informationspersonnelles') }}</span>
                        </div>
                        <div :class="{ 'text-white bg-[#94C12F] duration-500': step > 1, 'text-[#555555] bg-[#363636] duration-500': step == 1 }"
                            class="text-white bg-[#232323] font-medium py-2 px-5 text-sm rounded-lg cursor-pointer ">
                            <span>2. {{ $t('votrepointdedepart') }}</span>
                        </div>
                        <div :class="{ 'text-white bg-[#94C12F] duration-500': step > 2, 'text-[#555555] bg-[#363636] duration-500': step == 2 }"
                            class="text-white bg-[#232323] font-medium py-2 px-5 text-sm rounded-lg cursor-pointer ">
                            <span>3. {{ $t('votrenouvelleresidence') }}</span>
                        </div>

                        <div :class="{ 'text-white bg-[#94C12F] duration-500': step > 3, 'text-[#555555] bg-[#363636] duration-500': step == 3 }"
                            class="text-white bg-[#232323] font-medium py-2 px-5 text-sm rounded-lg cursor-pointer ">
                            <span>4. {{ $t('definissezlheurededepart') }}</span>
                        </div>

                        <div :class="{ 'text-white bg-[#94C12F] duration-500': step > 4, 'text-[#555555] bg-[#363636] duration-500': step == 4 }"
                            class="text-white bg-[#232323] font-medium py-2 px-5 text-sm rounded-lg cursor-pointer ">
                            <span>5. {{ $t('vosmeublesetautresademenager') }}</span>
                        </div>
                    </div>
                </div>
                <div id="screen" class="h-[80%]   w-full  duration-700">
                    <!-- formulaire -->
                    <div id="firstPage" :class="{ 'hidden': step != 0 }" class="w-full overflow-hidden  h-full  ">
                        <div class="w-full space-y-7 ">
                            <div class="flex flex-col w-full space-y-2 ">
                                <label for="name" class="text-white text-md ">{{ $t('quelestvotrenom') }}</label>
                                <input id="name" type="text" v-model="form.name" placeholder="John doe*"
                                    class="w-11/12  md:w-5/12 placeholder:text-md placeholder:font-medium pb-1 placeholder:text-[#2A2A2A] bg-transparent duration-500 border-b-2 outline-none font-semibold text-md text-[#94C12F] border-[#555555] focus:border-[#94C12F]"
                                    name="">
                            </div>
                            <div class="flex flex-col w-full space-y-2">
                                <label for="name" class="text-white text-md ">{{ $t('quelestvotreemail') }}</label>
                                <input type="email" v-model="form.email" placeholder="example@gmail.com"
                                    class="w-full md:w-5/12 text-[#94C12F] placeholder:text-md placeholder:font-medium pb-1 placeholder:text-[#2A2A2A] bg-transparent duration-500 border-b-2 outline-none font-semibold text-md  border-[#555555] focus:border-[#94C12F]"
                                    name="" id="">
                            </div>
                            <div class="flex flex-col w-full space-y-2">
                                <label for="name" class="text-white text-md ">{{ $t('quelestvotrenumero') }}</label>
                                <input v-model="form.phone" placeholder="11 digit phone number"
                                    style="-moz-appearance:textfield" type="number"
                                    class="w-full md:w-5/12 placeholder:text-md pb-1 placeholder:font-medium placeholder:text-[#2A2A2A] bg-transparent duration-500 border-b-2 outline-none font-semibold text-md text-[#94C12F] border-[#555555] focus:border-[#94C12F]"
                                    name="" id="">
                            </div>
                        </div>
                    </div>
                    <div :class="{ 'hidden': step != 1 }" class="w-full   overflow-y-scroll  h-full ">
                        <div class="w-full space-y-7 h-[90%]">
                            <div class="flex flex-col w-full space-y-2">
                                <label for="name" class="text-white text-md ">{{ $t('quelestvotrepaysactuelle') }}</label>
                                <input v-model="form.pays_depart" placeholder="deuschland*" type="text"
                                    class="w-full md:w-5/12 placeholder:text-md pb-1 placeholder:font-medium placeholder:text-[#2A2A2A] bg-transparent duration-500 border-b-2 outline-none font-semibold text-md text-[#94C12F] border-[#555555] focus:border-[#94C12F]"
                                    name="" id="">
                            </div>
                            <div class="flex flex-col w-full space-y-2">
                                <label for="name" class="text-white text-md ">{{ $t('quelestvotrevilleactuelle') }}</label>
                                <input v-model="form.ville_depart" placeholder="deuschland*" type="text"
                                    class="w-full md:w-5/12 placeholder:text-md pb-1 placeholder:font-medium placeholder:text-[#2A2A2A] bg-transparent duration-500 border-b-2 outline-none font-semibold text-md text-[#94C12F] border-[#555555] focus:border-[#94C12F]"
                                    name="" id="">
                            </div>
                            <div class="flex flex-col w-full space-y-2">
                                <label for="name" class="text-white text-md ">{{ $t('quelestvotredistrictactuelle')
                                }}</label>
                                <input v-model="form.quartier_depart" placeholder="quartier*" type="text"
                                    class="w-full md:w-5/12 placeholder:text-md pb-1 placeholder:font-medium placeholder:text-[#2A2A2A] bg-transparent duration-500 border-b-2 outline-none font-semibold text-md text-[#94C12F] border-[#555555] focus:border-[#94C12F]"
                                    name="" id="">
                            </div>

                            <div class="flex flex-col w-full space-y-2">
                                <label for="name" class="text-white text-md ">{{ $t('quelestladistancedelaroute') }}</label>
                                <input v-model="form.distance_route" placeholder="10cm*" style="-moz-appearance:textfield"
                                    type="number"
                                    class="w-full md:w-5/12 placeholder:text-md pb-1 placeholder:font-medium placeholder:text-[#2A2A2A] bg-transparent duration-500 border-b-2 outline-none font-semibold text-md text-[#94C12F] border-[#555555] focus:border-[#94C12F]"
                                    name="" id="">
                            </div>
                    ...
                    <div :class="{ 'hidden': step != 2 }" class="w-full  overflow-hidden  h-full ">
                        <div class="w-full  flex flex-col ">
                            <div class="space-y-7  h-[90%]  overflow-y-scroll">
                                <div class="flex flex-col w-full space-y-2">
                                    <label for="name" class="text-white text-md ">{{ $t('quelestvotrepaysdarrivee')
                                    }}</label>
                                    <input v-model="form.pays_destination" placeholder="deuschland*" type="text"
                                        class="w-full md:w-5/12 placeholder:text-md pb-1 placeholder:font-medium placeholder:text-[#2A2A2A] bg-transparent duration-500 border-b-2 outline-none font-semibold text-md text-[#94C12F] border-[#555555] focus:border-[#94C12F]"
                                        name="" id="">
                                </div>
                                <div class="flex flex-col w-full space-y-2">
                                    <label for="name" class="text-white text-md ">{{ $t('quelestvotrevilledarrivee')
                                    }}</label>
                                    <input v-model="form.ville_destination" placeholder="quartier*" type="text"
                                        class="w-full md:w-5/12 placeholder:text-md pb-1 placeholder:font-medium placeholder:text-[#2A2A2A] bg-transparent duration-500 border-b-2 outline-none font-semibold text-md text-[#94C12F] border-[#555555] focus:border-[#94C12F]"
                                        name="" id="">
                                </div>

                                <div class="flex flex-col w-full space-y-2">
                                    <label for="name" class="text-white text-md ">{{ $t('quelestvotredistrictdarrivee')
                                    }}</label>
                                    <input v-model="form.quartier_destination" placeholder="new york..."
                                        style="-moz-appearance:textfield" type="text"
                                        class="w-full md:w-5/12 placeholder:text-md pb-1 placeholder:font-medium placeholder:text-[#2A2A2A] bg-transparent duration-500 border-b-2 outline-none font-semibold text-md text-[#94C12F] border-[#555555] focus:border-[#94C12F]"
                                        name="" id="">
                                </div>
                                <div class="flex flex-col w-full space-y-2">
                                    <label for="name" class="text-white text-md ">{{ $t('quelestladistancedelaroute')
                                    }}</label>
                                    <input v-model="form.distance_route_destination" placeholder="100"
                                        style="-moz-appearance:textfield" type="text"
                                        class="w-full md:w-5/12 placeholder:text-md pb-1 placeholder:font-medium placeholder:text-[#2A2A2A] bg-transparent duration-500 border-b-2 outline-none font-semibold text-md text-[#94C12F] border-[#555555] focus:border-[#94C12F]"
                                        name="" id="">
                                </div>
                    ...
                    <div :class="{ 'hidden': step != 3 }" class="w-full  overflow-hidden  h-full ">
                        <div class="w-full    h-full flex flex-col  overflow-y-scroll">
                            <div class="space-y-7   ">
                                <div class="flex flex-col  w-full space-y-2">
                                    <label for="name" class="text-white text-md ">{{ $t('jourdepart') }}</label>
                                    <input v-model="form.date_demenagement" placeholder="quartier*" type="date"
                                        class="w-full md:w-5/12 placeholder:text-md pb-1 placeholder:font-medium placeholder:text-[#2A2A2A] bg-transparent duration-500 border-b-2 outline-none font-semibold text-md text-[#94C12F] border-[#555555] focus:border-[#94C12F]"
                                        name="" id="">
                                </div>

                                <div class="flex flex-col w-full space-y-2">
                                    <label for="name" class="text-white text-md ">{{ $t('heuredepart') }}</label>
                                    <input v-model="form.heure_demenagement" placeholder="quartier*" type="time"
                                        class="w-full md:w-5/12 placeholder:text-md pb-1 placeholder:font-medium placeholder:text-[#2A2A2A] bg-transparent duration-500 border-b-2 outline-none font-semibold text-md text-[#94C12F] border-[#555555] focus:border-[#94C12F]"
                                        name="" id="">
                                </div>
                            </div>
                    ...
                    <div :class="{ 'hidden': step != 4 }" class="w-full  overflow-hidden  h-full ">
                        <div class="w-full h-full justify-end flex flex-col  overflow-y-scroll overflow-hidden p-px ">
                            <div class="w-full h-[10%] flex space-x-5">
                                <div @click="switchType(0)" class="flex cursor-pointer space-x-2 h-full items-center">
                                    <div :class="move == 0 ? 'bg-[#94C12F] duration-700' : ' bg-white duration-700'"
                                        class="w-3 h-3  rounded-full"></div>
                                    <div :class="move == 0 ? 'text-[#94C12F] duration-700' : 'text-white duration-700'"
                                        class=" font-semibold">
                                        <p>{{ $t('elementnonfragiles') }}</p>
                                    </div>
                                </div>
                                <div @click="switchType(1)" class="flex cursor-pointer space-x-2 h-full items-center">
                                    <div :class="move == 1 ? 'bg-[#94C12F] duration-700' : ' bg-white duration-700'"
                                        class="w-3 h-3  rounded-full"></div>
                                    <div :class="move == 1 ? 'text-[#94C12F] duration-700' : 'text-white duration-700'"
                                        class=" font-semibold">
                                        <p>{{ $t('elementfragile') }}</p>
                                    </div>
                                </div>
                                <div @click="switchType(2)" class="flex cursor-pointer space-x-2 h-full items-center">
                                    <div :class="move == 2 ? 'bg-[#94C12F] duration-700' : ' bg-white duration-700'"
                                        class="w-3 h-3  rounded-full"></div>
                                    <div :class="move == 2 ? 'text-[#94C12F] duration-700' : 'text-white duration-700'"
                                        class=" font-semibold">
                                        <p>{{ $t('autres') }}</p>
                                    </div>
                                </div>
                            </div>
                            <div v-if="move == 0" :class="move == 0 ? 'h-full duration-700' : 'h-0 duration-700'"
                                class="h-full w-9/12 pr-3 overflow-hidden overflow-y-scroll ">
                                <div class="  w-full ">
                                    <div class=" mx-auto my-10">
                                        <ul class=" pb-5 space-y-2 divide-gray-200">

                                            <li @click="showModal('remove', item)" v-for="item in ObjectsNotFragiles"
                                                :key="item.id" class="  cursor-pointer group   w-full ">
                                                <div
                                                    class="w-full flex justify-between duration-700 group-hover:bg-white bg-[#94C12F] p-3 rounded ">
                                                    <div class="flex items-center">
                                                        <span class="ml-3 font-medium">{{ item.nom }}</span>
                                                    </div>
                                                    <div class="flex space-x-3">
                                                        <div>
                                                            <span
                                                                class="text-lg font-nunito text-white group-hover:text-[#94C12F]">{{
                                                                    $t('enregistre') }}</span>
                                                        </div>
                                                    </div>
                                                </div>
                                            </li>
                                            <li @click="showModal('add', item)" v-for="item in Objects" :key="item.id"
                                                class="  cursor-pointer group   w-full ">
                                                <div
                                                    class="w-full flex justify-between duration-700 group-hover:bg-[#94C12F] bg-white p-3 rounded ">
                                                    <div class="flex items-center">
                                                        <span class="ml-3 font-medium group-hover:text-white">{{ item.nom
                                                        }}</span>
                                                    </div>
                                                    <div class="flex space-x-3">
                                                        <div>
                                                            <span
                                                                class="text-lg font-nunito text-black group-hover:text-white">{{
                                                                    $t('nonselect') }}...</span>
                                                        </div>
                                                    </div>
                                                </div>
                            ...
                            <div v-if="move == 1" :class="move == 1 ? 'h-full duration-700' : 'h-0 duration-700'"
                                class="h-full w-9/12 pr-3 overflow-hidden overflow-y-scroll ">
                                <div class="  w-full ">
                                    <div class=" space-y-2 mx-auto flex flex-col">
                                        <div class="">
                                            <button @click="showModalFragile"
                                                class="px-4 py-2 rounded text-white bg-green-500 font-semibol">{{
                                                    $t('ajouter') }}</button>
                                        </div>
                                        <ul class=" pb-5 space-y-2 ">
                                            <li v-for="item in fragiles" :key="item.id"
                                                class="  cursor-pointer group   w-full ">
                                                <div
                                                    class="w-full flex justify-between duration-700 group-hover:bg-[#94C12F] bg-white p-3 rounded ">
                                                    <div class="flex items-center">
                                                    ...
                                                    <div class="flex space-x-3 ">
                                                        <div @click="deleteFragileItem(item.nom)"
                            ...
                            <div v-if="move == 2" :class="move == 2 ? 'h-full duration-700' : 'h-0 duration-700'"
                                class="h-full w-9/12 pr-3 overflow-hidden overflow-y-scroll ">
                                <div class="  w-full ">
                                    <div class=" space-y-2 mx-auto flex flex-col">
                                        <div class="">
                                            <button @click="showModalOther"
                                                class="px-4 py-2 rounded text-white bg-yellow-500 font-semibol">{{
                                                    $t('ajouter') }}</button>
                                        </div>
                                        <ul class=" pb-5 space-y-2 ">
                                            <li v-for="item in others" :key="item.id"
                                                ...
                                                <div
                                                    ...
                                                    <div class="flex space-x-3 ">
                                                        <div @click="deleteOtherItem(item.nom)"
                                                            class="text-black group-hover:text-white">
                                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none"
                                                                viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
                                                                class="w-6 h-6">
            ...
            <div class="mx-auto w-10/12 justify-end mt-1 flex space-x-3">
                <button v-if="step > 0 && step < 6" @click="PrevStep"
                    class="px-4 py-1 rounded text-[#94C12F] border border-[#94C12F] bg-[#171717] font-semibold">
                    {{ $t('precedent') }}
                </button>
                <button v-if="step < 6" @click="NextStep" class="px-4 py-1 rounded bg-[#94C12F] text-black font-semibold">
                    {{ $t('continu') }}
                </button>
            </div>
        </div>
...
export default {
    data() {
        return {
            ...
            type: '',
            loadModal: false,
            loadFragileModal: false,
            loadOtherModal: false,
            modalTap: false,
            modalFragileTap: false,
            modalOtherTap: false,
            modal: false,
            loadScreen: false,
            ObjectsNotFragiles: [],
    ...
    methods: {
        NextStep() {
            if (this.step == 0) {
                ...
                if (this.form.name.length == 0) {
                    window.emitter.emit("AlertError", [this.$i18n.messages[`${localStorage.getItem('langue')}`].votrenomestvide, true])
                } else if (!name.test(this.form.name)) {
                    window.emitter.emit("AlertError", [this.$i18n.messages[`${localStorage.getItem('langue')}`].votrenomenmajuscule, true])
                } else if (!this.form.email.includes("@") || !this.form.email.includes(".") || this.form.email.length == 0) {
                    window.emitter.emit("AlertError", ['Veuillez entrer un email valide', true])
                } else if (!regexPhone.test(this.form.phone)) {
                    window.emitter.emit("AlertError", ['Entrez un numero valide a 11 chiffres', true])
                } else {
                    this.step++;
                }
            } else if (this.step == 1) {
                if (this.form.ville_depart.length == 0) {
                    window.emitter.emit("AlertError", ['Veuillez entrer votre ville actuelle', true])
                } else if (this.form.quartier_depart.length == 0) {
                    window.emitter.emit("AlertError", ['Veuillez entrer votre quartier actuelle', true])
                } else if (this.form.distance_route.length == 0) {
                    window.emitter.emit("AlertError", ['Veuillez entrer a distance vous separant de la route', true])
                } else {
                    this.step++;
                }
            } else if (this.step == 2) {
                if (this.form.pays_destination.length == 0) {
                    window.emitter.emit("AlertError", ['Veuillez entrer votre pays de destination ', true])
                } else if (this.form.ville_destination.length == 0) {
                    window.emitter.emit("AlertError", ['Veuillez entrer votre ville de destination', true])
                } else if (this.form.quartier_destination.length == 0) {
                    window.emitter.emit("AlertError", ['Veuillez entrer votre nouveau quartier/distric', true])
                } else if (this.form.distance_route_destination.length == 0) {
                    window.emitter.emit("AlertError", ['Veuillez entrer la distance qui separe la route et votre nouvelle residence', true])
                } else {
                    this.step++;
                }
            } else if (this.step == 3) {
                if (this.form.date_demenagement.length == 0) {
                    window.emitter.emit("AlertError", ['Veuillez notez la date dont vous voulez debuter votre deplacement', true])
                } else if (this.form.heure_demenagement.length == 0) {
                    window.emitter.emit("AlertError", ['Veuillez notez l\'heure du demenagement ', true])
                } else {
                    this.step++;
                }
            } else if (this.step == 4) {
                if (this.ObjectsNotFragiles.length == 0) {
                    window.emitter.emit("AlertError", ['Vous n\'avez rien reference concernant les meubles a demenager', true])
                } else if (this.fragiles.length == 0) {
                    window.emitter.emit("AlertError", ['Vous n\'avez rien reference concernant les meubles fragiles a demenager ', true])
                } else if (this.others.length == 0) {
                    window.emitter.emit("AlertWarn", ['Vous n\'avez rien reference concernant les meubles fragiles a demenager ', true])
                } else {
                    this.move++;
                    this.step++;
                    this.modal = true
        ...
        SaveDem() {
            ...
            data.append("pays_depart", this.form.pays_depart);
            data.append("ville_depart", this.form.ville_depart);
            data.append("quartier_depart", this.form.quartier_depart);
            data.append("distance_route", this.form.distance_route);
            data.append("pays_destination", this.form.pays_destination);
            data.append("ville_destination", this.form.ville_destination);
            data.append("quartier_destination", this.form.quartier_destination);
            data.append("distance_route_destination", this.form.distance_route_destination);
            data.append("date_demenagement", this.form.date_demenagement);
            data.append("heure_demenagement", this.form.heure_demenagement);
            data.append("situation_local_depart", this.form.situation_local_depart);
            data.append("situation_local_arrivee", this.form.situation_local_arrivee);
            data.append("liaison_local_depart", this.form.liaison_local_depart);
            data.append("liaison_local_arrivee", this.form.liaison_local_arrivee);
            data.append("objects", JSON.stringify(this.ObjectsNotFragiles));
            data.append("object_fragiles", JSON.stringify(this.fragiles));
            data.append("others", JSON.stringify(this.others));

            this.axios.post(this.api + "MyDem", data)
                .then(({ data }) => {
                    this.loadScreen = false
                    window.emitter.emit("AlertValide", ['un email a ete envoye a l\'adresse indique', true])
                    this.$router.push('/')
                    console.log(data);
                }).catch(error => {
                    this.loadScreen = false
        ...
        showModal(type, item) {
            this.item.name = item.nom;
            this.type = type
            if (this.type == 'remove') {
                this.item.occ = item.occ
            }
        ...
        SaveItem() {
            let newItem = {};
            newItem.nom = this.item.name;
            newItem.occ = this.item.occ;
            this.ObjectsNotFragiles.unshift(newItem);
            this.$store.state.Objects.filter((item, index) => item.nom == this.item.name ? (this.$store.state.Objects.splice(index, 1)) : '')
            this.loadModal = true
            setTimeout(() => {
                this.item.name = ''
                this.item.occ = 1
        ...
        LeaveItem() {
            ...
            newItem.nom = this.item.name;
            newItem.occ = this.item.occ;
            this.$store.state.Objects.unshift(newItem);
            this.ObjectsNotFragiles.filter((item, index) => item.nom == this.item.name ? (this.ObjectsNotFragiles.splice(index, 1)) : '')
            this.loadModal = true
            setTimeout(() => {
                this.item.name = ''
                this.item.occ = 1
        ...
        // fragile
        showModalFragile() {
            this.modalFragileTap = true
        },

        addFragileItem() {
            if (this.item.name.length <= 3) {
                window.emitter.emit("AlertError", ['Veuillez entrer le nom de l\'element', true])
            } else {
                this.loadFragileModal = true
                if (this.ObjectsNotFragiles.filter((item) => item.nom == this.item.name)[0]) {
                    this.loadFragileModal = false
                    window.emitter.emit("AlertError", ['cette element existe deja dans la liste des objects non fragiles', true])
                } else if (this.Objects.filter((item) => item.nom == this.item.name)[0]) {
                    this.loadFragileModal = false
                    window.emitter.emit("AlertError", ['cette element existe, allez le selectionne dans la rubrique Elements non fragiles', true])
                } else {
                    let newItem = {};
                    newItem.nom = this.item.name;
                    newItem.occ = this.item.occ;
                    this.fragiles.filter((item, index) => item.nom == this.item.name ? (this.fragiles.splice(index, 1)) : '')
                    setTimeout(() => {
                        this.fragiles.unshift(newItem);
                        this.item.name = ''
                        this.item.occ = 1
                        this.loadFragileModal = false
                        this.modalFragileTap = false
                    }, 1000);
                }
        ...
        deleteFragileItem(name) {
            this.fragiles.filter((item, index) => item.nom == name ? (this.fragiles.splice(index, 1)) : '')
        },

        showModalOther() {
            this.modalOtherTap = true
        },
        ...
        addotherItem() {
            if (this.item.name.length <= 3) {
                ...
                this.loadOtherModal = true
                if (this.fragiles.filter((item) => item.nom == this.item.name)[0]) {
                    this.loadOtherModal = false
                    window.emitter.emit("AlertError", ['cette element existe deja dans la liste des objects fragiles', true])
                } else if (this.ObjectsNotFragiles.filter((item) => item.nom == this.item.name)[0]) {
                    this.loadOtherModal = false
                    window.emitter.emit("AlertError", ['cette element existe deja dans la liste des objects non fragiles', true])
                } else if (this.Objects.filter((item) => item.nom == this.item.name)[0]) {
                    this.loadFragileModal = false
                    ...
                    newItem.nom = this.item.name;
                    newItem.occ = this.item.occ;
                    this.others.filter((item, index) => item.nom == this.item.name ? (this.others.splice(index, 1)) : '')
                    setTimeout(() => {
                        this.others.unshift(newItem);
                        this.item.name = ''
                        this.item.occ = 1
                        this.loadOtherModal = false
                        this.modalOtherTap = false
                    }, 1000);
                }
        ...
        deleteOtherItem(name) {
            this.others.filter((item, index) => item.nom == name ? (this.others.splice(index, 1)) : '')
        },
    },
